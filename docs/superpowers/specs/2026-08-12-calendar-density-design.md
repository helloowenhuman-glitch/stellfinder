...
- If four or more event rows occur on a date, further rows are visually concealed by a white bottom gradient inside the cell.
- The lower-right corner of that cell shows a circular `+N` badge, where `N` is the number of concealed rows.
...
- Preserve the existing continuous weekly bars for multi-day events.
...
- Unit tests cover the current-day style, three-row rendering limit, hidden-row count, and no badge at three or fewer rows.
